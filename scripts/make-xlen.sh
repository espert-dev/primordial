#!/bin/sh
# Build system for Primordial (single XLEN).
#
...
AS="${AS:-riscv${XLEN}-unknown-elf-gcc}"
ASFLAGS="${ASFLAGS:-
	-DXLEN=${XLEN:-64}
	-ggdb3
	-Wa,--fatal-warnings
...
assemble lib/p0/format/newline.S
...
build_library lib/libp0.a \
	...
	"$BUILD_ROOT/lib/p0/format/newline.o" \
...
with_test lib/p0/format/newline_test \
	"$BUILD_ROOT/lib/libp0.a"

with_test lib/p0/format/signed_test \
	"$BUILD_ROOT/lib/libp0.a"

with_test lib/p0/format/space_test \
	"$BUILD_ROOT/lib/libp0.a"

with_test lib/p0/format/string_test \
	"$BUILD_ROOT/lib/libp0.a"
